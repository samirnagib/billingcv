...
public class ImportCommvaultFileController implements Initializable {
	...
	//variavel auxiliar para forma��o da competencia
	...
	private void btImportarOnAction (ActionEvent event) {
		...
		if (txtAno.getText()==null || txtAno.getText().trim().equals("")) {
			Alerts.showAlert("Mensagem de Erro", null, "O Campo Ano n�o pode estar vazio", AlertType.ERROR);
			txtAno.requestFocus();
		}
...
//					 System.out.println("----------------------");
				 ...
				 if ( numNvBT > 0 ) {
					 Alerts.showAlert("AVISO DE NOVA FAIXA DE COBRAN�A", null, "Foram encontrados " + numNvBT + " nova(s) faixas de cobran�a", AlertType.INFORMATION);
					 Alerts.showAlert("AVISO DE NOVA FAIXA DE COBRAN�A", "O relat�rio com as novas faixas est�o no caminho abaixo:", pathSave + "NovasFaixas.txt", AlertType.INFORMATION);
				 ...
				 if ( numNvCl > 0 ) {
					 ...
					 Alerts.showAlert("AVISO DE NOVO CLIENTE", "O relat�rio com os novos clientes est�o no caminho abaixo:", pathSave + "NovosClientes.txt", AlertType.INFORMATION);
				...
				lbMessages.setText("Importa��o concluida." );
			} //final do bloco try
		} //final do bloco if
